...
export default {
  ...
  methods: {
    ...
    async upDateTheme() {
      try {
        this.data = await axios.put('/upDate/Theme', {
          isTheme: this.isDarkTheme
        })
      } catch (err) {
        console.log('Error data is not updata', err)
      }
    }
  },
  mounted() {
...
<template>
  <div :class="{
    ...
    <div class="container-sb-2">
      ...
      <svg v-if="!active && !angle" class="change-color" xmlns="http://www.w3.org/2000/svg" id="Layer_1"
      ...
      <div v-if="!angle" @click="toggleTheme" class="toggle-sb-light">
        <input type="checkbox" @change="upDateTheme" class="toggler" :class="{ 'active': active }">
      </div>
    </div>
  </div>
</template>
